    ...
    return (
        <div id="slide_navbar" className={`h-screen w-screen fixed bg-white text-2xl font-semibold top-0 ${width}`}>
            ...
            <div className="h-4/5 flex flex-col items-center justify-center [&>a]:m-5">
                ...
                <Link to="about" onClick={() => {props.setVisible("hidden")}} smooth={true} duration={1000}>About</Link>
                <Link to="projects" onClick={() => {props.setVisible("hidden")}} smooth={true} duration={1000}>Projects</Link>
                <Link to="contact" onClick={() => {props.setVisible("hidden")}} smooth={true} duration={1000}>Contact</Link>
            </div>
        </div>
    );
}